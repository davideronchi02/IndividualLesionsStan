functions {
  ...
  for (j in 1:nSubjects_sub) {
    ...
    if(sum(livertumor) != 0) {livertumor = log(livertumor);} /// To be fixed ///
    if(sum(lungtumor) != 0) {lungtumor = log(lungtumor);}
    if(sum(lyntumor) != 0) {lyntumor = log(lyntumor);}
    if(sum(othertumor) != 0) {othertumor = log(othertumor);}
...
model{
  ...
  betaLIVER ~ normal(-0.5, 0.25);
  betaLUNG ~ normal(-0.5, 0.25);
  betaLYN ~ normal(-0.5, 0.25);
  betaOTHER ~ normal(-0.5, 0.25);
