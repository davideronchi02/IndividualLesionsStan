  ...
  for (j in 1:nSubjects_sub) {
    ...
    for(nl in 1:NLES[start_subject + j - 1]) {
       ...
       for (i in 1:15) {
        ...
        if(LESION[start_subject + j - 1,nl] == 1) {livertumor = livertumor + log(tumor[indexesGK]);}
        if(LESION[start_subject + j - 1,nl] == 2) {lungtumor = lungtumor + log(tumor[indexesGK]);}
        if(LESION[start_subject + j - 1,nl] == 3) {lyntumor = lyntumor + log(tumor[indexesGK]);}
        if(LESION[start_subject + j - 1,nl] == 4) {othertumor = othertumor + log(tumor[indexesGK]);}
