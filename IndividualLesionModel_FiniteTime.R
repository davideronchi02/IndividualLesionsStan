...
# Fix Error in patient "A6181170  137"
xdata[which(xdata$id ==  "A6181170  137" & xdata$TMMLNM == "T3"),]$TMMLNM = "T2"

xdata <- xdata |> filter(id != "A6181170  414") # patient has reported issues
...
loc <- matrix(as.numeric(!is.na(locator)),nrow=416,ncol=5)[,2:5] |> unname()
...
data <- list(nt = nt, # No. of Total Points
             ...
             amt = as.numeric(xdata$amount)/14, # From two weeks cumulative to right measurement unit
...
file <- file.path("Models/ModelMain_FiniteTime.stan")
